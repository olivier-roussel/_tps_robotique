...
def plotConvergence(log, pltAxes=None):
    ...
    plotConvergenceDetailed(
        log.costs, log.pregs, log.dregs, log.grads, log.stops, log.steps, pltAxes
    )


def plotConvergenceDetailed(costs, pregs, dregs, gamma, theta, alpha, pltAxes):
    """
    Plot the cost, regularization <pregs> (in us) and <dregs> (in xs), gradient
    ...
    :param pregs: regularization in us
    :param dregs: regularization in xs
    ...
    pltAxes[1].plot(pregs, label="primal regs")
    pltAxes[1].plot(dregs, label="dual regs")
    pltAxes[1].legend()
...
def displayTrajectory(viz, robot, xs, dt=0.01, rate=-1, tool_frame_id=-1, tool_frame_obj=None):
    ...
    for i, x in enumerate(xs):
        if not i % S:
            ...
            if tool_frame_id >= 0:
                pin.framesForwardKinematics(robot.model, robot.data, x[:viz.model.nq])
                oMf_tool = robot.data.oMf[tool_frame_id]
                viz.applyConfiguration(tool_frame_obj, oMf_tool)
            time.sleep(dt*S)
